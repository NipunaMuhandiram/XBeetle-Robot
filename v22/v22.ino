...
// Timing variables for counting interrupts
unsigned long lastMillis = 0;
unsigned long interval = 5000;  // 5 seconds interval

// Interrupt counters
volatile int frontInterruptCount = 0;
volatile int backInterruptCount = 0;
...
void loop() {
    ...
    // Reset the interrupt counts every 5 seconds
    if (currentMillis - lastMillis >= interval) {
        lastMillis = currentMillis;
        
        // Check if combined interrupt count exceeds 5
        if ((frontInterruptCount + backInterruptCount) > 5) {
            stopCar();
            turnRight(150);  // Turn right with speed 150
            Serial.println("Interrupts exceeded limit. Stopping and turning right.");
        }
        
        // Reset the counters after handling
        frontInterruptCount = 0;
        backInterruptCount = 0;
    }
